...
import { calculateTrafficWeight, calculateWindFactor, calculateTimeInSegment } from './weightFactors.js';
...
export async function calculateRoutePES(routeIndex, polyline) {
  ...
  for (const seg of enrichedSegments) {
    ...
    if (seg.traffic) {
        trafficWeight = calculateTrafficWeight(seg.traffic.free_flow_speed, seg.traffic.current_speed);
    }

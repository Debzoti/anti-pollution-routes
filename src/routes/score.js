...
router.post('/score', async (req, res) => {
  try {
    ...
    // Handle case where ORS returns fewer routes than expected
    if (!polylines || polylines.length === 0) {
      return res.status(404).json({ error: "No routes found between the specified coordinates" });
    }
    
    if (polylines.length < 3) {
      console.warn(`[score] ORS returned only ${polylines.length} route(s), expected 3`);
    }

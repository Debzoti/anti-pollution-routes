...
const POINTS = [{ lat: 28.6139, lon: 77.209, label: "New Delhi" }];
...
async function test() {
  ...
  try {
    ...
    if (w) {
      await writeRow(normaliseWeather(w));
      console.log("Weather: SUCCESS (Inserted 1 row)");
    }
  } catch (e) {
    console.log("Weather: ERR", e.message);
  }
  ...
  try {
    ...
    if (t) {
      await writeRow(normaliseTraffic(t, POINTS[0].lat, POINTS[0].lon));
      console.log("Traffic: SUCCESS (Inserted 1 row)");
    }
  } catch (e) {
    console.log("Traffic: ERR", e.message);
  }
